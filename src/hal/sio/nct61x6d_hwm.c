...
/** NCT61x6D Hwmon descriptor */
struct bctrl_desc *hal_hwm_desc(void);

/** Voltage (up to 8 physical channels) */
static const u8 hal_hwm_in_map[CONFIG_HWM_VOLTAGE_NUM] = CONFIG_HWM_VOLTAGE_MAP;
static struct hal_hwm_sensor nct61x6d_hwm_ins[CONFIG_HWM_VOLTAGE_NUM] = {
...
/** Temperature (up to 3 physical channels) */
static const u8 hal_hwm_temp_map[CONFIG_HWM_TEMPERATURE_NUM] =
	CONFIG_HWM_TEMPERATURE_MAP;
static struct hal_hwm_sensor nct61x6d_hwm_temps[CONFIG_HWM_TEMPERATURE_NUM] = {
...
/** Fan (up to 3 physical channels) */
static const u8 hal_hwm_fan_map[CONFIG_HWM_FAN_NUM] = CONFIG_HWM_FAN_MAP;
struct hal_hwm_sensor nct61x6d_hwm_fans[CONFIG_HWM_FAN_NUM] = {
	...
	HWM_SENSOR_ATTR(CONFIG_HWM_FAN_2_LABEL, HWM_SENSOR_TYPE_FAN_SPEED,
			CONFIG_HWM_FAN_2_REG_SPEED, CONFIG_HWM_FAN_2_SPEED_STEP,
			0, 0, hal_hwm_fan_read, NULL),
#else
	HWM_SENSOR_NONE,
#endif
...
/** PWM (up to 2 physical channels) */
static const u8 hal_hwm_pwm_map[CONFIG_HWM_PWM_NUM] = CONFIG_HWM_PWM_MAP;
struct hal_hwm_sensor nct61x6d_hwm_pwms[CONFIG_HWM_PWM_NUM] = {
...
/*
 * Select the HWM register bank (bit7 of 0x4E is preserved). Must be called
 * while the SIO ports point at the HWM base (i.e. after hal_hwm_init()).
 */
static void __nct61x6d_hwm_bank(u8 bank)
{
	u8 val = nct61x6d_dev.ops->read8(NCT61X6D_HWM_REG_BANK_SEL) & 0x80;
	nct61x6d_dev.ops->write8(NCT61X6D_HWM_REG_BANK_SEL, val | bank);
}
...
{
	return __get_hal_sensor(index, hal_hwm_in_map, nct61x6d_hwm_ins,
...
s32 hal_hwm_in_read(struct hal_hwm_sensor *sensor)
{
	...
	__nct61x6d_hwm_bank(NCT61X6D_HWM_BANK_VIN);
	raw = nct61x6d_dev.ops->read8(sensor->reg);
...
{
	return __get_hal_sensor(index, hal_hwm_temp_map, nct61x6d_hwm_temps,
...
s32 hal_hwm_temp_read(struct hal_hwm_sensor *sensor)
{
	...
	__nct61x6d_hwm_bank(NCT61X6D_HWM_BANK_TEMP);
	raw = nct61x6d_dev.ops->read8(sensor->reg);
	temp = raw * 1000;
...
{
	return __get_hal_sensor(index, hal_hwm_fan_map, nct61x6d_hwm_fans,
				CONFIG_HWM_FAN_NUM);
}
...
s32 hal_hwm_fan_read(struct hal_hwm_sensor *sensor)
{
	...
	__nct61x6d_hwm_bank(NCT61X6D_HWM_BANK_FAN_RPM);
	raw = nct61x6d_dev.ops->read16(sensor->reg);

	/* A stopped/absent fan reads 0 (or 0xFFFF); avoid divide-by-zero */
	if (raw == 0 || raw == 0xFFFF)
		return 0;

	speed = sensor->lsb / raw;

	log_debug("HAL HWM fan speed read[0x%02X]: raw=0x%04X, speed=%d RPM\n",
		  sensor->reg, raw, speed);
...
{
	return __get_hal_sensor(index, hal_hwm_pwm_map, nct61x6d_hwm_pwms,
				CONFIG_HWM_PWM_NUM);
}
...
s32 hal_hwm_pwm_read(struct hal_hwm_sensor *sensor)
{
	...
	__nct61x6d_hwm_bank(NCT61X6D_HWM_BANK_PWM);
	raw = nct61x6d_dev.ops->read8(sensor->reg);
		      ...
		      size_t count)
{
	...
	__nct61x6d_hwm_bank(NCT61X6D_HWM_BANK_PWM);
	nct61x6d_dev.ops->write8(sensor->reg, reg_value);
...
s32 hal_hwm_init(void)
{
	...
	ret = nct61x6d_dev.ops->probe();
	if (ret < 0) {
		log_err("NCT61x6D SIO probe failed\n");
		return ret;
	}

	nct61x6d_dev.ops->select(NCT61X6D_LD_HWM);

	addrh = nct61x6d_dev.ops->read8(NCT61X6D_HWM_REG_BASE_ADDR_H);
	if (addrh == 0xFF || addrh == 0x00) {
		log_err("NCT61x6D HWM base address high invalid: 0x%02X\n",
			addrh);
		return -ENODEV;
	}

	addrl = nct61x6d_dev.ops->read8(NCT61X6D_HWM_REG_BASE_ADDR_L);
	if (addrl == 0xFF) {
		log_err("NCT61x6D HWM base address low invalid: 0x%02X\n",
			addrl);
		return -ENODEV;
	}
	...
	if (addr == 0) {
		log_err("NCT61x6D HWM base address is zero\n");
		return -ENODEV;
	}

	// Switch the SIO ports to the HWM index/data pair (base + 5 / + 6)
	nct61x6d_dev.cfg->index_port = addr + 5;
	nct61x6d_dev.cfg->data_port = addr + 6;

	log_debug("NCT61x6D HWM at I/O 0x%04X (index 0x%04X, data 0x%04X)\n",
		  addr, nct61x6d_dev.cfg->index_port,
		  nct61x6d_dev.cfg->data_port);

	// debug: dump each configured channel
	for (i = 0; i < CONFIG_HWM_VOLTAGE_NUM; i++) {
		log_debug("HAL HWM voltage sensor %d: label=%s, reg=0x%02X\n", i,
			  nct61x6d_hwm_ins[i].label, nct61x6d_hwm_ins[i].reg);
		hal_hwm_in_read(&nct61x6d_hwm_ins[i]);
	}

	for (i = 0; i < CONFIG_HWM_TEMPERATURE_NUM; i++) {
		log_debug("HAL HWM temperature sensor %d: label=%s, reg=0x%02X\n",
			  i, nct61x6d_hwm_temps[i].label,
			  nct61x6d_hwm_temps[i].reg);
		hal_hwm_temp_read(&nct61x6d_hwm_temps[i]);
	}

	for (i = 0; i < CONFIG_HWM_FAN_NUM; i++) {
		log_debug("HAL HWM fan sensor %d: label=%s, reg=0x%02X\n", i,
			  nct61x6d_hwm_fans[i].label,
			  nct61x6d_hwm_fans[i].reg);
		hal_hwm_fan_read(&nct61x6d_hwm_fans[i]);
	}

	for (i = 0; i < CONFIG_HWM_PWM_NUM; i++) {
		log_debug("HAL HWM PWM sensor %d: label=%s, reg=0x%02X\n", i,
			  nct61x6d_hwm_pwms[i].label,
			  nct61x6d_hwm_pwms[i].reg);
		hal_hwm_pwm_read(&nct61x6d_hwm_pwms[i]);
	}
