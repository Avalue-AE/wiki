...
| Ubuntu (fetched `linux-headers` package, not under `/kernels`) | 4.15.0-101 -- EPC-WHL | Builds clean | A real, complete 4.15 tree [recipe](test/Test-infrastructure), built one subsystem at a time: this board declares watchdog, gpio and hwmon (no misc). All 3 produced a `.ko`, `make` exited 0 for each, and every `.ko`'s `modinfo -F vermagic` reads `4.15.0-101-generic SMP mod_unload`. |
